---
title: "Présentation des données - projet-pilote de personnes-ressources - an 1"
...
```{r socio, fig.show="hold", fig.align='center', out.width='30%', fig.cap='Exemples de sociogrammes (extraits anonymisés)'}
...
```{r vis2, out.width = "80%", fig.align='center', fig.cap='distribution des scores', echo=FALSE, dpi=300}
be_vis
```
...
quali_force <- quali %>% 
  select(d)
fl2 <- flextable::flextable(quali_force)
fl2 <- flextable::set_header_labels(fl2, "d" = "Forces")
...
quali_faibl <- quali %>% 
  select(e)
fl3 <- flextable::flextable(quali_faibl)
fl3 <- flextable::set_header_labels(fl3, "e" = "Limites et remédiations")
...
fl4 <- flextable::flextable(quali_rem)
fl4 <- flextable::set_header_labels(fl4, "f" = "Améliorations possibles")
